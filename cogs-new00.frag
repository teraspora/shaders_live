...
void main(void) {
    ...
    uv /= scale;
    // =======================================================================
    float t =  800. + u_time * .4;
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // MUTABLE PARAMETERS:
    float blackThreshold = 0.8 + 2. * nmouse().y; // 1.1; // Best between 1.0 and 1.3
    float rotationSense1 = 1.;  //  + 0.1 *sin(length(uv));
    float rotationSense2 = -1.; //  - 0.1 *sin(length(uv));
    // ==================================================================
    vec3 col; //  = 0.5 + 0.5 * cos(t / 17. + uv.xyx + vec3(1. * cos(t / 9.), 2. * cos(t / 3.), 4.));
    
    uv *= 1. + vec2(0.5 + nsin(uv.y * t / (6. + 2. * nsin(t / 24.))) * 0.4, 0.4 + nsin(uv.x * t / 32.) * 3.1);
    uv = rotate(uv, t * 0.7 * sign(sin(arg(uv) * 13.)));
    uv = rotate(uv - .03, t * 0.3 * sign(sin(arg(uv) * 23.)));
    uv = rotate(uv + .05, t * 0.7 * sign(sin(arg(uv) * 13.)));
    
    // col *= 1. - cog_saw(uv, 2. * nsin(t, 128.0, 2.4));
    vec2 st = uv; // + vec2(-0.6, -0.5);
    st = rotate(st, -t / 2.);    
    float m = 1. - cog_saw(st, 0.2, 6., 0.11);
    col *= vec3(m);

    vec2 uvn = uv + vec2(asp, 1.) / 2.;
    uvn.x /= asp;
    // vec2 st = (uv * scale + vec2(asp, 1.) / 2.) / scale;

    if (u_tex0Resolution != vec2(0.0) ) {
        vec4 img0 = texture2D(u_tex0, uvn);
        col = img0.rgb;
    }
    col.g = clamp(col.g * nsin(arg(uv) * 13.) * 4., 0., 1.);
    ...
    if (u_tex1Resolution != vec2(0.0) && col == black) {
            // st += vec2(0.0005 * sin(abs(st.y) * t / 9700.), 0.0007 * sin(abs(st.x) * t / 2800.));
            vec4 img1 = texture2D(u_tex1, st);
            col = img1.rgb;
