...
#define BEAT 0.42857142857142855
...
void main(void) {
    float t = u_time / 3.;
    float scale = 0.5;
    if (u_time > 25. && u_time < 80. && mod(u_time, BEAT) < BEAT / 16.) {
        scale *= 1.2;
    }
    if (u_time > 68. && u_time < 80. && mod(u_time, BEAT) < BEAT / 16.) {
        scale.x *= 2.;
    }
    ...
    vec2 uv = (2. * gl_FragCoord - u_resolution.xy) / (u_resolution.y * scale);
