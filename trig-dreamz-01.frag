...
void main(void) {
 ...
 //    for (int i = 0; i < 5; i++) {
 //      	 vec2 pos = vec2(asp * (gold_noise(vec2(floor(u_time / 64.), 2. * floor(u_time / 64.)), 1.)),
 //                         gold_noise(vec2(floor(u_time / 60.), sin(floor(u_time / 60.) / 61.)), 43.)) - 0.5;
 //         col += vec3(1., 0.3, 0.) * step(distance(st, pos), 0.08);
	// }
