...
vec3 partSaturate(vec3 col, float saturation) {
    vec3 colHSL = rgb2hsl(col);
    colHSL.t = saturation;
    return hsl2rgb(colHSL);    
}
...
void main(void) {
    // float scale = 1.; // 0.1 + 12. * nmouse().x;
    // float scale =  2.;
    // MUTABLE PARAMETERS:
    float blackThreshold = 0.4 + 2. * nmouse().y; // 1.1; // Best between 1.0 and 1.3
    float rotationSense1 = 1.;  //  + 0.1 *sin(length(uv));
    float rotationSense2 = -1.; //  - 0.1 *sin(length(uv));
    // ==================================================================
    float t = (u_time / (2. + 0.2 * cos(u_time / 128.))) * 2. ;
    float scale = pow(2., 3. * sin(t / 8.) - 7.);
    scale += 8. * nmouse().x;
    // if (t > 120.) {
    //     scale = pow(2., t - 125.);
    // }
...
// =======================================================================
    ...
    uv = rotate(uv, mod(t, 64.) > 32. ? t : -t);
    vec2 mouse = 2. * u_mouse.xy / u_resolution.xy - 1.;
    mouse.x *= u_resolution.x / u_resolution.y;
    uv = mod(t, 16.) > 8. ? uv : abs(uv);

    bool condition0 = false; // mod(t, 48.) > 32.;
    bool condition1 = false; // mod(t, 31.) > 19;
    ...
    // Uncomment the following line sometimes
    //uv = mix(uv, polar(.1 / abs(uv), arg(uv)), nsin(t / 7.));
    ...
    pq.x /= asp;
    pq.y += 0.12 * cos(t / 2.) * sin(t * pq.x / 32.);   // ) * sin(mod(pq.x * t, 7.));
    pq = rotate(pq, t / 2.) * cos(t / 6.);
    

    int i = int(mod(floor(u_time / 48.), 9.));
    switch (i) {
        case 0:
    ...
    float cdelta = mod(u_time / 3., 8.) / 8.;
    
    // col = partSaturate(col, nsin(t / 4.) );
    col = changeHue(col, fract(hue(col) + cdelta)); 
    // col = mix(col, saturate(col), nsin(t / 4.));
