...
void main(void) {
    ...
    float scale =  16384. * nmouse().x / numPoints / u_resolution.x;
    ...
    //if (nmouse().x >= 0.5) 
    //uv = abs(uv);
    //uv = rotate(uv, u_mouse.y * u_time / 1024.);

    vec2 friz = vec2(64. * sin(t * uv.x), 160. * cos(t * uv.y));
    vec2 vel = vec2(1., 10.);
    
    uv *= vec2(op(sin(friz.x * cos(vel.x * t) * uv.x) / 7.), op(cos(friz.y * sin(vel.y * t) * uv.y) / 4.)); 
    ...
    float c = mod(u_time, 128.) - 104.;
    uv = rotate(uv, -c * PI / (uv.x * scale < 0.0 + 32. * sin(uv.y * scale * 6.) / 4. ? -8. : 8.));
    uv = rotate(uv, c * PI / (uv.y * scale < 0.0 + 2. * sin(uv.x * scale * 3.) / 8. ? -12. : 12.));
    ...
    col.b -= nsin(t + length(uv * 10.));
    col.g += 0.5 * smoothstep(0., 1., col.r + col.b);
    
    // if (rgb2hcv(col).z < 0.2) col = 0.5 + 0.5 * cos(4. * t + uv.xyx + vec3(27, 49, 81));
