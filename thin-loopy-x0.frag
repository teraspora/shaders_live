...
float shape(vec2 u, float minkOrder, float time, float polygonOrder) {
    ...
    float r = minkl(u, minkOrder) + 0.9 * sin(abs(18. * (arg(u))) * polygonOrder * sin(u_time / time));
    return s(r, 0.9) - s(r, 0.1);    
}
...
void main(void) {
    ...
    float m = shape(rotate(uv, 2. * PI * sin(t * 128.)) * (1. + 0.2 * sin(t / 3. - 0.0)), 1., 43., 3. * nsin(t * 2.125));
    float n = shape(rotate(uv, -2. * PI * sin(t * 49.)) * (1. + 0.2 * sin(t / 11. + 0.0)), 1., 17., 5. * nsin(t * 2.06));
    float p = shape(rotate(uv, 2. * PI * sin(t * 17.)) * (1. + 0.2 * sin(t / 17. - 0.0)), 2., 13., 7. * nsin(t * 2.037));
    //float q = shape(uv + 0.0, 3., 8.);
    
    col = vec3(n + p, m + n, p +m);
