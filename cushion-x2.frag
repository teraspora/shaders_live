...
float f(float x) {
	return sin(cos(6.2 * u_time * x)) + cos(2. * sin(0.23 * u_time * x)) - sqrt(0.2 *abs(x) * cos(0.02 * u_time * x));
}
...
void main(void) {
    ...
    vec2 friz = vec2(0.8 * sin(t * uv.x), 0.8 * cos(t * uv.y));
    vec2 vel = vec2(1., 8.);
    ...
    uv = rotate(uv, rotationSense1 * c * PI / (uv.x * scale < 0.0 + 2. * sin(uv.y * scale * 12.) / 4. ? -8. : 8.));
    uv = rotate(uv, rotationSense2 * c * PI / (uv.y * scale < 0.0 + 2. * sin(uv.x * scale * 36.) / 8. ? -12. : 12.));
    ...
    if (rgb2hcv(col).z < 0.5) col = 0.5 + 0.5 * cos(4. * t + uv.xyx + vec3(27,49,81));
    
    
    col *= step(0., blackThreshold - length(col));
        
    gl_FragColor = vec4(col.gbr, 1.);
}
